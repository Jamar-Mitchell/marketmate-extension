...
const createMockListing = (overrides: Partial<ListingData> = {}): ListingData => ({
  id: 'test-123',
  title: 'Test Item',
...
describe('Pricing Engine', () => {
  describe('analyzePricing', () => {
    ...
    it('should increase flexibility for older listings', () => {
      ...
      expect(
        ['medium', 'high'].includes(oldAnalysis.flexibility)
      ).toBe(true);
    });
    ...
    it('should generate pricing factors', () => {
      ...
      expect(analysis.factors.some(f => f.name === 'Time on Market')).toBe(true);
    });
