...
test.describe('Full Negotiation Flow', () => {
  test('should complete full negotiation from listing to accepted', async ({ page, context }) => {
    ...
    await messengerPage.evaluate(() => {
      localStorage.setItem('marketmate-negotiation', JSON.stringify({
        id: 'test-neg-1',
        ...
        maxPrice: 200,
        counterHistory: [
          { amount: 195, fromSeller: true, timestamp: new Date().toISOString() }
        ],
        messages: [],
      }));
  ...
  test('should walk away when seller counter exceeds max', async ({ page, context }) => {
    ...
    await messengerPage.evaluate(() => {
      localStorage.setItem('marketmate-negotiation', JSON.stringify({
        ...
        maxPrice: 180,
        counterHistory: [
          { amount: 200, fromSeller: true, timestamp: new Date().toISOString() },
          { amount: 175, fromSeller: false, timestamp: new Date().toISOString() },
          { amount: 195, fromSeller: true, timestamp: new Date().toISOString() },
        ],
        messages: [],
      }));
