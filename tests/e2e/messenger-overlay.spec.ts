  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem('marketmate-negotiation', JSON.stringify({
        ...
        maxPrice: 200,
        counterHistory: [
          { amount: 220, fromSeller: true, timestamp: new Date().toISOString() }
        ],
        messages: [],
      }));
    });
...
test.describe('Messenger Overlay - Walk Away', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem('marketmate-negotiation', JSON.stringify({
        ...
        counterHistory: [
          { amount: 220, fromSeller: true, timestamp: new Date().toISOString() },
          { amount: 195, fromSeller: false, timestamp: new Date().toISOString() },
          { amount: 215, fromSeller: true, timestamp: new Date().toISOString() },
        ],
        messages: [],
      }));
    });
  ...
  test('should show walk away option when max price exceeded', async ({ page }) => {
    const sendBtn = page.locator('[data-testid="mm-send-suggestion"]');
