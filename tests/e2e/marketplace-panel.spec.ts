...
test.describe('Marketplace Panel', () => {
  ...
  test('should collapse panel when clicking collapse button', async ({ page }) => {
    // First expand
    await page.locator('[data-testid="mm-expand-btn"]').click();
    await expect(page.locator('[data-testid="mm-panel-expanded"]')).toBeVisible();
    
    // Then collapse
    await page.locator('[data-testid="mm-collapse-btn"]').click();
    await expect(page.locator('[data-testid="mm-panel-collapsed"]')).toBeVisible();
  });
