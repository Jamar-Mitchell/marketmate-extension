...
const CONDITION_KEYWORDS = {
  excellent: ['like new', 'mint', 'excellent', 'perfect', 'pristine', 'brand new', 'sealed'],
  good: ['good', 'great', 'works perfectly', 'fully functional', 'clean'],
  fair: ['fair', 'used', 'some wear', 'minor scratches', 'small dent'],
  poor: ['broken', 'damaged', 'parts only', 'for parts', 'not working', 'needs repair'],
};
...
function parseDaysListed(timeText: string): number {
  ...
  // Just listed / today
  if (lowerText.includes('just') || lowerText.includes('today') || lowerText.includes('hour')) {
    return 0;
  }
 ...
 */
function detectCondition(text: string): { condition: string; keywords: string[] } {
  ...
  return { condition: foundKeywords.length > 0 ? 'mixed' : 'used', keywords: foundKeywords };
}
...
export function extractListingData(): ListingData | null {
  try {
    ...
    // Condition
    const { condition, keywords: conditionKeywords } = detectCondition(fullText);
    ...
    return {
      ...
      category,
      location,
      timeListed: daysListed > 0 ? new Date(Date.now() - daysListed * 24 * 60 * 60 * 1000) : null,
      daysListed,
      condition,
      conditionKeywords,
      urgencyIndicators,
...
export function getMockListingData(): ListingData {
  return {
    ...
    description: 'Used PS5 in great condition. Includes one controller and all cables. Must sell, moving next week. No lowballers please.',
    askingPrice: 250,
    currency: 'USD',
 ...
 */
export function observeListingChanges(callback: (listing: ListingData | null) => void): MutationObserver {
