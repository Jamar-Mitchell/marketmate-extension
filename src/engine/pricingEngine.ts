...
function calculateFlexibility(listing: ListingData): 'low' | 'medium' | 'high' {
  ...
  // Urgency indicators
  const urgentKeywords = ['must sell', 'moving', 'desperate', 'asap', 'quick sale', 'need gone'];
  ...
  // OBO or negotiable
  if (listing.urgencyIndicators.some(ind => 
    ['obo', 'or best offer', 'negotiable', 'make offer'].includes(ind.toLowerCase())
  )) {
    score += 2;
  }
...
export function analyzePricing(listing: ListingData): PriceAnalysis {
  ...
  // 2. Condition factor
  const conditionMultiplier = CONDITION_MULTIPLIERS[listing.condition] || CONDITION_MULTIPLIERS.used;
  ...
  const depreciation = Object.entries(CATEGORY_DEPRECIATION).find(
    ([cat]) => categoryLower.includes(cat)
  ...
  const urgentCount = listing.urgencyIndicators.filter(ind => 
    !['firm', 'no lowballers'].includes(ind.toLowerCase())
  ).length;
  ...
  const priceRoundness = listing.askingPrice % 100 === 0 ? 0.02 : 
                         listing.askingPrice % 50 === 0 ? 0.01 : 0;
  if (priceRoundness > 0) {
  ...
  const recommendedDiscount = flexibility === 'high' ? 0.3 :
                              flexibility === 'medium' ? 0.25 : 0.2;
  const recommendedOffer = Math.round(listing.askingPrice * (1 - recommendedDiscount));
  ...
  return {
    ...
    recommendedOffer: Math.max(recommendedOffer, Math.round(listing.askingPrice * 0.6)),
    flexibility,
    confidenceScore,
    factors,
  };
