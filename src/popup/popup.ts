...
document.addEventListener("DOMContentLoaded", async () => {
  const mockModeToggle = document.getElementById(
    "mock-mode"
  ) as HTMLInputElement;
  const autoSuggestToggle = document.getElementById(
    "auto-suggest"
  ) as HTMLInputElement;
  ...
  if (stored.preferences) {
    ...
    autoSuggestToggle.checked =
      stored.preferences.automationLevel !== "suggest-only";
  }
  ...
  if (stored.currentNegotiation) {
    ...
    if (statusText)
      statusText.textContent = `Negotiating: $${neg.currentOffer}`;
  }
  ...
  autoSuggestToggle?.addEventListener("change", async () => {
    ...
    const updated = {
      ...current.preferences,
      automationLevel: autoSuggestToggle.checked
        ? "one-click-send"
        : "suggest-only",
    };
    await chrome.storage.local.set({ preferences: updated });
